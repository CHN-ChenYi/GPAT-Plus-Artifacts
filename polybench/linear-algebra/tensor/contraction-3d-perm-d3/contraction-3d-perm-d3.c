...
#include <string.h>
#include <math.h>
#include "contraction-3d-perm-d3.h"
#include <polybench.h>
...
static
void kernel_contraction_3d_perm_d3(int a_d_1, int a_d_2, int a_d_3,
...
#pragma scop
    ...
    for (int i = 0; i < _PB_A_D_3; i++) {
        for (int k = 0; k < _PB_B_D_3; k++) {
            for (int m = 0; m < _PB_A_D_1 / 3; m++) {
                for (int n = 0; n < _PB_A_D_2; n++) {
                    C[i][k] += A[3 * m][n][i] * B[n][3 * m][k];
                }
            }
        }
    }
...
int main(int argc, char** argv)
{
  ...
  /* Run kernel. */
  kernel_contraction_3d_perm_d3(a_d_1, a_d_2, a_d_3,
                           b_d_1, b_d_2, b_d_3,
                           POLYBENCH_ARRAY(C),
                           POLYBENCH_ARRAY(A),
                           POLYBENCH_ARRAY(B));
