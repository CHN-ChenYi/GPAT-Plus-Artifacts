...
function checkDependencies() {
  if [[ -z $MLIR_PLUS_OPT || ! -f $MLIR_OPT ]]; then
    echo "mlir-opt-plus not found!"
    exit 1
  fi
...
for i in $(find $INPUT_DIR -name "*.mlir" | sort); do
  FNAME=$(basename $i)
  echo -e " Polymer (polymer-opt-plus) $FNAME"
  ...
  if [ $out_code -eq 124 ]; then
    echoRed "  (polymer-opt-plus) Timeout: $FNAME" | tee -a $OUTPUT_DIR/error.txt
    continue
  fi
  if [ $out_code -ne 0 ]; then
    echoRed "  (polymer-opt-plus) Error: $FNAME" | tee -a $OUTPUT_DIR/error.txt
    continue
  fi
  ...
  if [ $? -ne 0 ]; then
    echoRed "  (mlir-opt-plus) Error: $FNAME" | tee -a $OUTPUT_DIR/error.txt
    continue
  fi
