...
function checkDependencies() {
  ...
  if [[ -z $MLIR_PLUS_TRANSLATE || ! -f $MLIR_PLUS_TRANSLATE ]]; then
    echo "mlir-plus-translate not found!"
    exit 1
  fi
...
for i in $(cat $POLYBENCH_BENCHMARK_LIST); do
  ...
  echo -e " Lowering and compiling (mlir-plus-opt mlir-plus-translate clang) $FNAME and ${FNAME%.*}.mlir"
  cp $INPUT_DIR/${FNAME%.*}.mlir $OUTPUT_DIR/${FNAME%.*}.mlir

  # Lower affine to the llvm dialect
  $MLIR_PLUS_OPT $OUTPUT_DIR/${FNAME%.*}.mlir \
